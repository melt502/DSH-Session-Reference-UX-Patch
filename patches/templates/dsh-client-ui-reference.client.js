		...
		function apply(ctx) {
			...
			const source = {
				...
				async candidates(session, { query, quoted, drilled, signal }) {
					...
					return [...visibleFiles.flatMap((candidate) => fileCandidate(candidate, quoted === true, withLocation, t)), ...visibleSessions.map((candidate) => sessionCandidate(candidate, listed[candidate.sessionId]?.updatedAt ?? candidate.createdAt, now, home, t))];
				},
				header(_session, req) {
