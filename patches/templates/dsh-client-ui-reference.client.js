	...
	factory: (require) => {
		...
		*/
		...
		const zh = {
			...
			"candidate.currentWorkspace": "当前工作区",
			"candidate.otherWorkspace": "其他工作区",
		...
		const en = {
			...
			"candidate.session": "Session",
			"candidate.currentWorkspace": "Current workspace",
			"candidate.otherWorkspace": "Other workspace",
		...
		*/
		function apply(ctx) {
			...
			const source = {
				...
				async candidates(session, { query, quoted, signal }) {
					...
					const visibleFiles = emptyQuery ? fileItems.slice(0, 8) : fileItems;
					const visibleSessions = emptyQuery ? sessionItems.slice(0, 8) : sessionItems;
					return [...visibleFiles.flatMap((candidate) => fileCandidate(candidate, quoted === true, t)), ...visibleSessions.map((candidate) => sessionCandidate(candidate, t))];
				},
				onPick({ candidate }) {
		...
		function sessionCandidate(candidate, t) {
			...
			const fallbackLabel = candidate.label.trim() === "" || candidate.label === candidate.sessionId;
			const displayLabel = fallbackLabel ? `${candidate.sessionId.slice(0, 18)}${candidate.sessionId.length > 18 ? "…" : ""}` : candidate.label;
			const description = [location, dateLabel].filter(Boolean).join(" · ");
			const value = {
				kind: "session",
				label: displayLabel,
			...
			return {
				name: displayLabel,
				description,
				title: `${candidate.label} · ${candidate.sessionId}`,
