		...
		*/
		...
		function InputBar({ useSession, useInput, inputActions, keyboard, addImages, removeImage, draftImages, resolveSubmitMode, toggleCommandMenu, stop, command, t, renderSlot, useNotices, useLexicon, useMenuLauncher, useProjection, sessionId, variant, disabled: inert = false, blocked, workspacePickerOpen = false, onRequestWorkspace, placeholder, accessory, overlay, leftItems, rightItems, footer }) {
			...
			const parseSessionDrop = (dataTransfer) => {
				if (!Array.from(dataTransfer.types).includes(SESSION_REFERENCE_MIME)) return;
				try {
			...
			const onSessionDrop = (e) => {
				...
				if (!accepted) return;
				const occurrence = keyboard.snapshot.occurrences.find((item) => item.offset === selection.start && item.ref === payload.mention);
				const caret = occurrence === void 0 ? selection.start : occurrence.offset + occurrence.length + (keyboard.snapshot.draft[occurrence.offset + occurrence.length] === " " ? 1 : 0);
